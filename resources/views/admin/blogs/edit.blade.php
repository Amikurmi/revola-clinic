...
<div class="max-w-3xl mx-auto py-10 px-6">
    ...
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-3xl font-bold text-[#3c2c20]">Edit Blog</h1>
    ...
    <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $blog->title) }}"
                class="w-full border border-gray-300 px-3 py-2 rounded focus:outline-none focus:ring focus:ring-[#8b6f47]" required>
        </div>
        ...
        <div>
            ...
            <textarea name="excerpt" rows="3"
                class="w-full border border-gray-300 px-3 py-2 rounded focus:outline-none focus:ring focus:ring-[#8b6f47]">{{ old('excerpt', $blog->excerpt) }}</textarea>
        </div>

        <div>
            <label class="block mb-2 font-semibold">Content <span class="text-red-600">*</span></label>
            <textarea id="editor" name="content" rows="6"
                class="w-full border border-gray-300 px-3 py-2 rounded focus:outline-none focus:ring focus:ring-[#8b6f47]" required>{{ old('content', $blog->content) }}</textarea>
        </div>

        <div>
            <label class="block mb-2 font-semibold">Current Image</label>
            @if($blog->image)
            <img src="{{ asset('storage/' . $blog->image) }}" class="w-32 h-32 object-cover rounded-lg border mb-2">
            @else
